...
await writeFile(path.join(outDir, 'narration-scripts.json'), `${JSON.stringify(scripts, null, 2)}\n`, 'utf8');

for (const item of scripts) {
  const output = path.join(outDir, `slide-${String(item.id).padStart(2, '0')}.wav`);
  console.log(`Generating ${path.basename(output)}`);
  const result = spawnSync(
    'supertonic',
    ['tts', item.text, '-o', output, '--lang', 'ko', '--voice', 'F2', '--steps', '8', '--max-chunk-length', '180'],
    {
      cwd: root,
      encoding: 'utf8',
